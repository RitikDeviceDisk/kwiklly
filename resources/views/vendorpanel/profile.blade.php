...
<form action="{{ route('vendor.updateProfile') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="row">
        {{-- Store Open Time --}}
        <div class="col-3">
            <label for="open_time">Store Open Time</label>
        </div>
        <div class="col-6">
            <select name="open_time" id="open_time" class="form-control">
                <option value="">Select Open Time</option>
                @foreach($store_times as $time)
                    <option value="{{ $time->slot_time }}" {{ $vendor->open_time == $time->slot_time ? 'selected' : '' }}>{{ $time->slot_time }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="row">
        {{-- Store Close Time --}}
        <div class="col-3">
            <label for="close_time">Store Close Time</label>
        </div>
        <div class="col-6">
            <select name="close_time" id="close_time" class="form-control">
                <option value="">Select Close Time</option>
                @foreach($store_times as $time)
                    <option value="{{ $time->slot_time }}" {{ $vendor->close_time == $time->slot_time ? 'selected' : '' }}>{{ $time->slot_time }}</option>
                @endforeach
            </select>
        </div>
    </div>
